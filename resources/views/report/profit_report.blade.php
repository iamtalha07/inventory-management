                        ...
                        <div class="row">
                            ...
                            <div class="col-md-1">
                                <div class="form-group">
                                    ...
                                    <button type="submit" class="btn btn-success search-btn" style="margin-top: 35px;"><i
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title titleclass">Report</h3>
                            <div class="card-tools">
                                <div class="row">
                                    <div class="input-group input-group-sm" style="width: 50px;">
                                        <a href="javascript:void(0)" title="Print" onclick="summaryPrint('report-print')"
                                            class="btn btn-block btn-info"><i class="fas fa-print"></i></a>
                                    </div>&nbsp
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tbody tbody id="leadsTable">
                                            ...
                                            @foreach ($invoice->invoiceProduct as $invoiceProduct)
                                                <tr>
                                                    ...
                                                    <td>{{ $invoiceProduct->pivot->unit_price ?? $invoiceProduct->ctn_per_piece_price }}</td>
                                                    <td>{{ $invoiceProduct->purchase_rate }}</td>
                                                    <td>{{ $invoiceProduct->pivot->qty ?? $invoiceProduct->pivot->ctn_qty * $invoiceProduct->ctn_size }}</td>
                                                    <td>{{ $invoiceProduct->pivot->amount }}</td>
                                                    <td>{{ $invoice->created_at->format('Y-m-d') }}</td>
                                                    <td>{{ $invoiceProduct->profit }}</td>
                                                </tr>
                                            @endforeach
                                        @endforeach
                                        </tbody>
